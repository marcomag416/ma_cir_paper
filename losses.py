...
def compute_MA_mono_sw_loss(delta_features, target_features, logit_scale, lambd=1, sim_grad=True, similarity="none"):
    ...
    target_features = make_normalized(target_features)
    delta_features = make_normalized(delta_features)
...
def compute_unslerp_mono_loss(delta_features, target_features, logit_scale, alpha=0.8, sim_grad=True, similarity="none"):
    N = delta_features.shape[0]
    device = delta_features.device
    beta = 1/alpha # extrapolation factor

    source_i = target_features.unsqueeze(1)                # (N, 1, D)
    delta_j = delta_features.unsqueeze(0)                 # (1, N, D)
    delta_i = delta_features.unsqueeze(1)               # (N, 1, D)

    #NOTE: normalization is done inside slerp

    #extrapolate delta features
    ext_delta = slerp(delta_i, delta_j, beta)  # (N, N, D)
    if ext_delta.isnan().any():
        raise ValueError("NaN values found in extrapolated delta features")
    #interpolate with source features
    y = slerp(source_i, ext_delta, alpha)          # (N, N, D)
    if y.isnan().any():
        raise ValueError("NaN values found in unslerp output features")


    query = y.reshape(-1, y.shape[-1])  # (N*N, D)
    ...
    #weights (w_ij)= text_features[i]*text_features[j] for each i,j
    features_for_similarity = make_normalized(features_for_similarity)  # Normalize features
    self_similarities = features_for_similarity @ features_for_similarity.T  # (N, N)
    weights = (self_similarities * (self_similarities > 0).int()).pow(2).flatten()  # Ensure non-negative weights, (N*N,)
